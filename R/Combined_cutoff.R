#' Identification of Careless Reponse Using Standard Deviation and Time per Item
#' 
#' \code{Combined_cutoff} returns dataframe of assessment flagged by two cutoff values: Standard Deviation per assessment and Time per Item. 
#' 
#' This function creates the dataframe that includes the ID and index of assessments that met the cutoff criteria for \emph{Standard Deviation per assessment} as well as \emph{Time per Item}. If an assessment has a Standard Deviation \strong{less than or equal to} the cutoff value and a Time per Item \strong{less than or equal to} the cutoff value, it will be flagged and placed in the dataframe. This function uses the Boolean logic \emph{"AND"} in order to identify assessments that meet both criteria.
#'
...
Combined_cutoff <- function(data, SD.cutoff, TPI.cutoff, ttc.colnames, number.items, mandatory.response, item.colnames, ID.colname){
