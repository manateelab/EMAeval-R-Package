...
#'
#' This function creates the dataframe that includes the ID and the percentage of assessments that met the cutoff criteria for \emph{Standard Deviation per assessment}, \emph{Time per Item}, and \emph{Percent of Items at Mode }.
...
Combined_cutoff_percent <- function(data, SD.cutoff, SD.condition, TPI.cutoff, TPI.condition, Perc.Mode.cutoff, Perc.Mode.condition, Combined.logic, ttc.colnames, number.items, mandatory.response, item.colnames, ID.colname){
  ...
  for(idx in 1:nrow(data)){
    ...
    if (length(item.colnames[!is.na(item.colnames)]) == 0){
      ...
      Number_items_Mode <- length(which(data[idx, item.colnames] == mode[1]))
